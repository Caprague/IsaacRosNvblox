...
def generate_launch_description() -> LaunchDescription:
    ...
    args.add_arg('camera_serial_numbers', '336222074023,242622071196',
                 description='List of the serial no of the extra cameras. (comma separated)',
                 cli=True)
    args.add_arg(
        'multicam_urdf_path',
        lu.get_path('nvblox_examples_bringup',
                    'config/urdf/2_realsense_go2_camera0_base.urdf.xacro'),
    ...
    actions.append(imu_horizontal_align_node)
    ...
    actions.append(
        lu.include(
            'nvblox_examples_bringup',
            'launch/perception/nvblox_realsense.launch.py',
            launch_arguments={
                'container_name': args.container_name,
                'mode': args.mode,
                'camera': camera_mode,
    ...
    # TF transforms for multi-realsense (always load for proper TF tree)
    actions.append(
