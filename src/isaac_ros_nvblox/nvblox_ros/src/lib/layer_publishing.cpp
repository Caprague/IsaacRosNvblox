  ...
  const CudaStream& cuda_stream) 
{
  ...
  if (layers_to_stream & LayerType::kHeightScan) {
    timing::Timer publish_timer("ros/publish_navigation_height_scan");
    publishNavigationHeightScan_impl(
...
void LayerPublisher::publishNavigationHeightScan_impl(
...
{ 
  // 采样参数
  const float range_x = 4.6f;
  const float range_y = 3.0f;
  const float resolution = 0.2f;
  const float x_offset = 0.7f;
  const float y_offset = 0.0f;
  const float z_offset = 0.5f;
  const float max_casting_depth = 2.0f;
  const int x_steps = static_cast<int>(range_x / resolution) + 1;
  const int y_steps = static_cast<int>(range_y / resolution) + 1;
  
  // 存储网格点高程采样结果
  navigation_sample_points_x.clear();
  navigation_sample_points_y.clear();
  navigation_sample_points_z.clear();
  ...
  ray_caster_->sampleTerrainPoints(
    // 采样点坐标(采样结果)
    navigation_sample_points_x,
    navigation_sample_points_y,
    navigation_sample_points_z,
    // 体素栅格地图
    tsdf_layer,
    // 采样参数
    base_pose,
  ...
  // 发布高程采样结果(仅z值)
  std_msgs::msg::Float32MultiArray navigation_height_scan_msg;
  navigation_height_scan_msg.layout.dim.clear();
  auto dim_x = std_msgs::msg::MultiArrayDimension();
  dim_x.label = "x";
  dim_x.size = x_steps;
  dim_x.stride = x_steps * y_steps;  // 从起点到下一行的元素数
  navigation_height_scan_msg.layout.dim.push_back(dim_x);
  auto dim_y = std_msgs::msg::MultiArrayDimension();
  dim_y.label = "y";
  dim_y.size = y_steps;
  dim_y.stride = y_steps;  // 从行起点到下一个列元素
  navigation_height_scan_msg.layout.dim.push_back(dim_y);
  navigation_height_scan_msg.data = navigation_sample_points_z;
  navigation_height_scan_msg.layout.data_offset = 0;  // 数据起始偏移
  navigation_hs_publisher_->publish(navigation_height_scan_msg);

  // 可视化高程采样结果(三维Rviz)
  sensor_msgs::msg::PointCloud2 navigation_hs_pc_msg;
  navigation_hs_pc_msg.header.frame_id = "odom_horizontal";
  navigation_hs_pc_msg.header.stamp = rclcpp::Clock().now();
  sensor_msgs::PointCloud2Modifier modifier(navigation_hs_pc_msg);
  modifier.setPointCloud2FieldsByString(1, "xyz");
  modifier.resize(navigation_sample_points_x.size());
  sensor_msgs::PointCloud2Iterator<float> iter_x(navigation_hs_pc_msg, "x");
  sensor_msgs::PointCloud2Iterator<float> iter_y(navigation_hs_pc_msg, "y");
  sensor_msgs::PointCloud2Iterator<float> iter_z(navigation_hs_pc_msg, "z");
  for (size_t i = 0; i < navigation_sample_points_x.size(); ++i, ++iter_x, ++iter_y, ++iter_z) 
  {
      *iter_x = navigation_sample_points_x[i];
      *iter_y = navigation_sample_points_y[i];
      *iter_z = navigation_sample_points_z[i];
  }
  navigation_hs_pc_publisher_->publish(navigation_hs_pc_msg);
}
  ...
  locomotion_y_steps_(static_cast<int>(locomotion_range_y / locomotion_resolution) + 1)
{
  ...
  // heightscan 发布
  locomotion_hs_publisher_ =
      node->create_publisher<std_msgs::msg::Float32MultiArray>(
          "~/locomotion_height_scan", 1);
  navigation_hs_publisher_ =
      node->create_publisher<std_msgs::msg::Float32MultiArray>(
          "~/navigation_height_scan", 1);

  // 可视化发布
  locomotion_hs_pc_publisher_ = 
      node->create_publisher<sensor_msgs::msg::PointCloud2>(
          "~/locomotion_height_scan_pc", 1);
